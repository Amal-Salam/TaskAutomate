  ...
  handler: async (ctx, args) => {
    ...
    const storyPoints =
      args.storyPoints != null
        ...
        : undefined;
    const inventoryItems = validateInventoryItems(args.inventoryItems);
    ...
    const taskId = await ctx.db.insert('tasks', {
      ...
      storyPoints,
      inventoryBlocked: args.inventoryBlocked ?? false,
      inventoryItems: inventoryItems ?? [],
      parentGoal: args.parentGoal,
      isDecomposed:args.isDecomposed,
      createdAt: Date.now(),
      createdBy: user._id,
    });
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const { user } = await requireMember(ctx, task.workspaceId);
    ...
    await ctx.db.patch(taskId, { ...fields, title, description,inventoryItems, storyPoints });
  },
});
